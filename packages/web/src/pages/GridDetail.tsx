...
export function GridDetail() {
  ...
  return (
    <>
      ...
      <GroupsCard grid={grid} onChanged={refresh} />
...
// --- Image groups on this grid -------------------------------------------------------

function GroupsCard({ grid, onChanged }: { grid: Grid; onChanged: () => void }) {
  const notify = useNotify()
  const queryClient = useQueryClient()
  const { data: groups } = useQuery({
    queryKey: ['image-groups', grid.id],
    queryFn: () => api.listImageGroups(grid.id),
    refetchInterval: 30_000,
  })
  const [confirmDelete, setConfirmDelete] = useState<string | null>(null)

  const refresh = () => {
    queryClient.invalidateQueries({ queryKey: ['image-groups', grid.id] })
    queryClient.invalidateQueries({ queryKey: ['grid-queue', grid.id] })
    onChanged()
  }

  const remove = async (groupId: string, deleteImages: boolean) => {
    setConfirmDelete(null)
    try {
      await api.deleteImageGroup(groupId, deleteImages)
      notify(deleteImages ? 'Group and its images deleted.' : 'Group dissolved — images stay in the library.', 'positive')
      refresh()
    } catch (err) {
      notify(`Delete failed: ${errMessage(err)}`, 'negative')
    }
  ...
  // Membership edits replace the full ordered id list (the API contract).
  const setMembers = async (groupId: string, imageIds: string[], message: string) => {
    try {
      await api.updateImageGroup(groupId, { image_ids: imageIds })
  ...
  return (
    <div className="ink-card" style={{ gap: 12 }}>
      <h3 className="ink-h3">Grouped images</h3>
      <span className="ink-small">
        Groups play as a unit in the queue: slot-addressed screens show one per panel, plain images one per refresh.
        Create groups from the Images page (select images → Group…).
      </span>
      {groups.map((group) => (
        <div
          key={group.id}
          ...
          style={{ borderTop: '1px solid var(--ink-border)', paddingTop: 10 }}
        >
          <div className="row w-full items-center gap-2 wrap">
            <span className="ink-body" style={{ fontWeight: 600 }}>
              {group.name}
            </span>
            {group.display_job_id && <Badge tone="accent">generated</Badge>}
            <span className="ink-small">
              {group.images.length} image(s) ·{' '}
              {group.last_displayed_at ? `shown ${formatRelative(group.last_displayed_at)}` : 'not shown yet'}
            </span>
            <div className="flex-1" />
            <Button flat danger round icon="delete" title="Delete group" onClick={() => setConfirmDelete(group.id)} />
          </div>
          <div className="row gap-2 wrap">
            {group.images.map((image, index) => (
              <div key={image.id} className="col gap-1" style={{ alignItems: 'center' }}>
                <img
                  src={mediaUrl(image.storage_path, 240)}
                  alt={image.title ?? ''}
                  style={{ width: 72, height: 54, objectFit: 'cover', borderRadius: 6 }}
                />
                {/* Generated groups are ephemeral worker output — their
                    slot screens are not hand-editable. */}
                  ...
                  <div className="row gap-0">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setMembers(group.id, ids, 'Order saved.')
                      }}
                    />
                    <Button
                      flat
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setMembers(
                          group.id,
                          group.images.filter((img) => img.id !== image.id).map((img) => img.id),
          ...
          {!group.display_job_id && (
            <span className="ink-small" style={{ opacity: 0.7 }}>
              Add images from the Images page: select them → Group… → “Add to: {group.name}”.
            </span>
          )}
          <ConfirmDialog
            open={confirmDelete === group.id}
            message={
              group.display_job_id
                ? `Delete generated group '${group.name}' and its screens?`
                : `Dissolve group '${group.name}'? Its images return to the library (they are not deleted).`
            }
            destructive
            confirmLabel={group.display_job_id ? 'Delete' : 'Dissolve'}
            onConfirm={() => remove(group.id, Boolean(group.display_job_id))}
