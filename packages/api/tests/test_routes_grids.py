...
class TestGridCrud:
    ...
    def test_update_display_schedule_round_trip(self, client: TestClient, seed_device: Device):
        ...
        assert body["display_cron"] == "30 7 * * 1,2,3,4,5"
        assert body["display_timezone"] == "Europe/Berlin"
        assert body["display_duration_seconds"] == 3600
        # Enabling stamped the lease with the next occurrence.
        assert body["display_next_at"] is not None

        # Disabling clears the lease but keeps the cron for re-enabling.
        resp = client.put(f"/api/grids/{grid_id}", json={"display_schedule_enabled": False})
        assert resp.json()["display_next_at"] is None
        assert resp.json()["display_cron"] == "30 7 * * 1,2,3,4,5"

        resp = client.put(f"/api/grids/{grid_id}", json={"clear_display_duration": True})
        assert resp.json()["display_duration_seconds"] is None

    def test_update_rejects_bad_timezone_and_cron(self, client: TestClient, seed_device: Device):
        ...
        assert client.put(f"/api/grids/{grid_id}", json={"display_cron": "25:00"}).status_code == 422
