...
devices whose last refresh failed; a never-acked device (last_refresh_ok is
None) stays eligible, and recovery is automatic once a success ack clears the
flag.
"""
...
def _due_device(profile: DeviceProfile, device_id: str, *, last_refresh_ok: bool | None) -> Device:
    return Device(
        id=uuid4(),
        device_id=device_id,
