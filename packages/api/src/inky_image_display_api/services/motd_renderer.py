...
def render_image_screen(ai_image_bytes: bytes, width: int, height: int) -> bytes | None:
    """Cover-fit the AI-generated illustration to the panel."""
    return ImageProcessor.process_for_display(ai_image_bytes, width, height, upscale=True)


class StoryContent(Protocol):
    """The story fields the renderer needs — any object with these attrs.

    Decouples rendering from persistence: the worker POSTs the story in
    the render request body, nothing is read from the database.
    """
...
def render_part(
    ...
    QR without a source URL, image without generated bytes, or empty text.
    """
    if part_key == "image":
        if ai_image_bytes is None:
            return None
        return render_image_screen(ai_image_bytes, width, height)
