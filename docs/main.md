...
- **Display jobs**: `/api/display-jobs/*` are content generators that target a grid — the MOTD (daily positive story generated with Gemini, optionally grounded in Google Search) is the first job type. A job maps content parts (What/Why/When, AI image, QR source link, takeaway) onto grid slots and renders exact-size screens on its own interval (same cadence model as the sync jobs, but run in-process). *When* the content shows is the grid's business: each grid has a daily display schedule and session state, takes its panels over at display time with the newest ready content from the jobs targeting it, and on release resumes rotation at jittered times so panels don't flash in lockstep. A single display participates by living in a one-panel grid. See [motd.md](motd.md).
