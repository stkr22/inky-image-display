...
| `/grids/{id}` | Grid detail: proportional canvas preview with drag-to-reposition placements (numeric cm inputs remain for precision), confirmation before removing a placement, the "Scheduled content" card (display-job schedule: 24h time, weekdays, timezone, duration or "until released"; live session status with per-slot parts and Show now / Release), per-grid image pool with resolution traffic-light badges and an honest truncation note |
| `/jobs` | Tabbed list of Immich sync, Gemini and display jobs (`?tab=gemini\|display`) with per-job "Last run" summaries (from `/api/sync-runs` for the worker jobs) and a "Run now" button, plus each job's schedule ("Runs every … · next …"). Create/edit forms live at `/sync-jobs/*`, `/gemini-jobs/*` and `/display-jobs/*` |
| `/display-jobs/{id}` | Display-job editor: story prompt + source mode + image preset, generation interval (blank = manual), target grid with one content part per slot, generate/display actions, and a 7-day story history with per-story preview and redisplay (see [motd.md](motd.md)). Session control lives on the grid page |
